...
const Plans = () => {
  ...
  useEffect(() => {
    const getPlan = async () => {
      ...
      const response = await axios.get('http://54.183.167.79:3001/api/plan',
  ...
  const deletePlan = async (plan) => {
    axios.delete(`http://54.183.167.79:3001/api/plan/${plan}`, { headers: { Authorization: `Bearer ${localUser.token}` } })
      .then(res => {
        console.log(res)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    await axios.put(`http://54.183.167.79:3001/api/plan/${plan._id}`, plan, { headers: { Authorization: `Bearer ${localUser.token}` } })
      .then(res => {
        console.log(res)
        window.location.reload()
      })
